"""
Base DEX Scanner — Fixed invalid addresses + expanded testnet DEXes (4 DEXes)
Auto flash provider: Balancer V2 (0%) → Aave V3 (0.05%) → Uniswap V3 Flash
"""
...
# --- Testnet DEXes: 4 DEXes so cross-DEX arb is possible on testnet ---
DEX_CONFIGS_TESTNET = {
...
BASE_TOKENS_TESTNET = {k: v for k, v in BASE_TOKENS_MAINNET.items()}
BASE_TOKENS_TESTNET['USDC'] = '0x036CbD53842c5426634e7929541eC2318f3dCF7e'
...
# All addresses manually verified — no spaces, all 42 chars
QUOTE_TOKENS = {
...
QUOTE_TOKENS = {k: v for k, v in QUOTE_TOKENS.items() if _valid_addr(v)}
...
class BaseScanner:
    ...
    def _dex_configs(self):
        return DEX_CONFIGS_TESTNET if self.testnet else DEX_CONFIGS_MAINNET
    ...
    def _get_eth_price(self):
        ...
        try:
            factory = DEX_CONFIGS_MAINNET.get('BaseSwap', {}).get('factory', '')
            weth = BASE_TOKENS_MAINNET.get('WETH', '').lower()
            usdc = BASE_TOKENS_MAINNET.get('USDC', '').lower()
            if factory and weth and usdc:
                res = self._multicall([(factory, _enc_get_pair(weth, usdc))])
                if res and res[0][0]:
            ...
            for q_addr_raw in quote_src.values():
                ...
                for dex, cfg in dex_cfgs.items():
                    ...
                    if key not in self._pair_cache:
                        to_discover.append((base_addr, q, dex, cfg['factory'], key))
        if to_discover:
    ...
    def execute_trade(self, opportunity, wallet_address, contract_address):
        try:
            ...
            return {'status':'ready','unsignedTx':{
                'to':tx['to'],'data':tx['data'],'gas':hex(tx['gas']),
                'gasPrice':hex(tx['gasPrice']),'nonce':hex(tx['nonce']),
                'value':'0x0','chainId':84532 if self.testnet else 8453,
            }}
        except Exception as e:
