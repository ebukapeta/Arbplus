...
def _build_pool_list():
    ...
    for (t0, t1, dex), pool in KNOWN_POOLS.items():
        if len(pool.strip()) != 42 or not pool.startswith("0x"):
...
class BSCScanner:
    ...
    def _batch_get_reserves(self, pool_addrs: list) -> dict:
        ...
        """
        valid = [p for p in pool_addrs if self._is_valid_addr(p)]
        if not valid or not self._mc:
            return {}

        calls = [
            (Web3.to_checksum_address(p.lower()), True, bytes.fromhex(GET_RESERVES_SIG[2:]))
            for p in valid
        ]
    ...
    def _batch_get_token0(self, pool_addrs: list) -> dict:
        unknown = [p for p in pool_addrs if self._is_valid_addr(p) and p.lower() not in self._pool_token0]
        if not unknown or not self._mc:
            return self._pool_token0

        calls = [
            (Web3.to_checksum_address(p.lower()), True, bytes.fromhex(TOKEN0_SIG[2:]))
            for p in unknown
        ]
        try:
            results = self._mc.functions.aggregate3(calls).call()
