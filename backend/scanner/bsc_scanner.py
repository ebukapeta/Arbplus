            ...
            for i in range(len(dex_names)):
                for j in range(len(dex_names)):
                    ...
                    if not result.get('profitable') or result.get('optimal_amount', 0) <= 0:
                        ...
                        peak   = result.get('peak_net_usd', 0)
                        if gap > -1.0 or peak > 0:
                            near_misses.append({
                    ...
                    loan_tok  = result['optimal_amount'] / 1e18
                    gross_tok = result['gross_profit']   / 1e18
                    net_tok   = result['net_profit']     / 1e18
                    ...
                    if net_usd <= 0:
                        near_misses.append({
                            'pair': pair_key, 'buy': buy_dex, 'sell': sell_dex,
                            'spread': spread, 'hurdle': fee_hurdle, 'gap': gap,
                            'reason': f'gas eats profit (net_usd=${net_usd:.2f}, loan=${loan_usd:.0f})',
                        })
                        continue
