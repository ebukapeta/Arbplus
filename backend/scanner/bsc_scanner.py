"""
BSC DEX Scanner — 100 Quote Tokens + Gas-Aware Trade Sizing
"""
...
QUOTE_TOKENS = {
    ...
    # Meme tokens — highest cross-DEX spread
    ...
    'STKBNB':  '0xc2E9d07048AB0697E25dBBf4B9B4c7C8A826E5f0',
    ...
    'CRV':     '0x98cc3bd6af1880fcfDa17ac477b2F612980e5e33',
    'BAL':     '0x04756126F044634C9a0f0E985e60c88a51ACC206',
    'GRT':     '0x52Ce071Bd9b1C4B00A0b92D298c512478CaD67e8',
    'ZRX':     '0x3Fb787101DC6Be47cfe18aeEe15404dcC842e6AF',
    'OXT':     '0x9aab071b4129B083B01cB5a0Cb513Ce7eca26fa5',
    'REN':     '0x695AbEE2db3CcF54AB416CeA2Db6d27f18B1AcAE',
...
def _sort_tokens(addr_a: str, addr_b: str):
    """UniswapV2: token0 is always the lower address."""
...
class BSCScanner:
    ...
    def _discover_pools(self, base_tokens: list, selected_dexes: list) -> dict:
        ...
        if to_discover:
            ...
            logger.info(f"Discovery: {found} real pools found from {len(calls)} queries")
        else:
    ...
    def _get_gas_price_gwei(self) -> float:
        """Get current BSC gas price in gwei from chain."""
        try:
            gwei = self.w3.eth.gas_price / 1e9
            # Clamp to sensible BSC range: 1-5 gwei
            return max(1.0, min(5.0, float(gwei)))
        except Exception:
            return 1.5  # BSC default
    ...
    def scan(self, config: dict) -> dict:
        ...
        min_net_profit_pct = float(config.get('minNetProfitPct', 0.05))
        min_liquidity_usd  = float(config.get('minLiquidityUsd', 2000))
        ...
        logger.info(f"Gas: {gas_gwei:.1f} gwei, BNB ${bnb_price:.0f} → gas cost ${gas_usd:.3f}")
        ...
        logger.info(f"Pairs ≥1 DEX: {len(pair_data)} | ≥2 DEXes (arb candidates): {len(pairs_multi)}")
        ...
        for pair_key, pdata in pairs_multi.items():
            ...
            for i in range(len(dex_names)):
                for j in range(len(dex_names)):
                    ...
                    # CORRECT: profit when buy_spot > sell_spot
                    # buy DEX gives MORE quote per base = cheaper to buy quote there
                    # sell DEX gives LESS quote per base = better to sell quote there
                    ...
                    result = find_optimal_trade_size(
                        ...
                        max_price_impact_pct=5.0,   # allow up to 5% price impact
                        decimals_base=18,
                        gas_usd=gas_usd,             # floor: loan must cover gas
                        base_price_usd=price_usd,
                    )

                    # Compute all values regardless of profitability
                    loan_tok  = result.get('optimal_amount', 0) / 1e18
                    ...
                    net_usd   = peak_usd - gas_usd   # peak already post-fee, subtract gas
                    ...
                    flash_fee_usd  = (result.get('flash_fee', 0) / 1e18) * price_usd
                    total_dex_fees = (loan_usd * (bd['fee_bps'] / 10000) +
                                      gross_usd * (sd['fee_bps'] / 10000))

                    # Status: profitable only if optimizer approved AND covers gas AND meets min %
                    is_profitable = (result.get('profitable', False)
                                     and net_usd > 0
                                     and net_pct >= min_net_profit_pct)
                    ...
                    if not is_profitable:
                        near_misses.append({
                            ...
                            'peak_net_usd': peak_usd,
                            'reason': result.get('reason', ''),
                        })

        # Diagnostics
        if near_misses:
            near_misses.sort(key=lambda x: x.get('peak_net_usd', 0), reverse=True)
            logger.info(f"Near-misses — top 10 (sorted by peak profit before gas):")
            for nm in near_misses[:10]:
                logger.info(
                    f"  {nm['pair']:22s}  {nm['buy']} → {nm['sell']}"
                    f"  spread={nm['spread']:.4f}%  gap={nm['gap']:+.4f}%"
    ...
    def execute_trade(self, opportunity: dict, wallet_address: str, contract_address: str) -> dict:
        try:
            ...
            flash_amount = int(opportunity['flashLoanAmount'] * 1e18)
